...
struct Point {
    ...
    int cluster_id;
    ClusterState state;
    Point(double x, double y, double z) : x(x), y(y), z(z), cluster_id(0), state(UNCLASSIFIED) {}
...
std::vector<int> create_cluster(const int curr_index, const std::vector<Point>& points, const double epsilon) {
...
void nearest_neighbour_search(const int curr_index, std::vector<Point>& points, const double epsilon, const int min_points, const int cluster_id) {
    ...
    if (cluster_seeds.size() < min_points) {
        ...
        for (auto index:cluster_seeds) {
            if (points[index].state != CLASSIFIED) {
        ...
        while (i < cluster_seeds.size()) {
            ...
            points[cluster_seeds[i]].state = CLASSIFIED;
            points[cluster_seeds[i]].cluster_id = cluster_id;
...
std::unordered_map<int, std::vector<Point>> create_cluster_map(std::vector<Point>& points, const double epsilon, const int min_points) {
